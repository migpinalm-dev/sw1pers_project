...
def plot_score_landscape(scores, score_density, finer_spline, window_size, window_slide, dates, rolling_size_scores):
    ...
    ax2 = ax1.twinx()
    bar_y = scores
    bar_x = np.arange(offset, offset + window_slide * len(scores), window_slide)
...
def plot_scores_comparison(scores, secondary_scores, window_size, window_slide, dates, factor=1, score_rolling_size=1, sec_score_rolling_size=1, is_score_windowed = False, show_bars = False, label_msg = "Alternative anomaly score"):
    fig, ax1 = plt.subplots(figsize=(11, 4))
    offset = round(window_size/2)

    # Window x-axis set up -------------------------
    windows_x = np.arange(offset, offset + window_slide * len(scores), window_slide)
    
    # Secondary scores plot ----------------
    sec_score_density = gaussian_density(secondary_scores, sec_score_rolling_size)
    if is_score_windowed:
        ...
        x = range(0, len(gaussian_density(secondary_scores, sec_score_rolling_size)))
    ...
    ma_y = gaussian_density(scores, score_rolling_size)[-int(-score_rolling_size//2) : int(-score_rolling_size//2)]    # math.ceil(score_rolling_size//2)) also works here
